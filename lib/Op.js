...
class Op extends React.PureComponent {
    ...
    componentWillMount() {
        const initialState = new OpState(this);
        ...
        if (this.props.autoInvoke) {
            ...
            initialState.invoke(input);
        }

        this.setState({
            currentOp: initialState
        });
    }
    ...
    _invoke(op, input, resultCb) {
        ...
        // (no error if no currentOp yet, i.e. autoInvoke)
        if (this._isUnmounted || (this.state.currentOp && this.state.currentOp !== op)) {
