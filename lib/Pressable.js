...
class PressState {
    constructor(domNode, domDownEvent, domPosition, initialEvent, initialPosition) {
        this.node = domNode;
...
function createPressAreaComponent(parentNode) {
    return class PressArea extends React.PureComponent {
        ...
        _onMouseDown(domNode, domDownEvent) {
            ...
            parentNode._startMousePress(domNode, domDownEvent);
        }

        _onTouchStart(domNode, domTouchStartEvent) {
            domTouchStartEvent.preventDefault();

            parentNode._startTouchPress(domNode, domTouchStartEvent);
        }
...
class Pressable extends React.PureComponent {
    ...
    _startMousePress(domNode, domDownEvent) {
        // mouse event contains the DOM position itself
        const pressState = new PressState(domNode, domDownEvent, domDownEvent, null, null);
        ...
        const onMove = (moveEvent) => {
            ...
            this._updatePressState(
                pressState,
                new PressState(domNode, moveEvent, moveEvent, domDownEvent, domDownEvent)
            );
        };
    ...
    _startTouchPress(domNode, domTouchStartEvent) {
        ...
        const pressState = new PressState(domNode, domTouchStartEvent, domTouchStartPosition, null, null);
        ...
        const onMove = (moveEvent) => {
            ...
            this._updatePressState(
                pressState,
                new PressState(domNode, moveEvent, matchingTouch, domTouchStartEvent, domTouchStartPosition)
            );
        };
