...
function createPointerAreaComponent(parentNode) {
    return class PointerArea extends React.PureComponent {
        constructor(props) {
            super();
        ...
        _onTouchStart(domNode, domTouchStartEvent) {
            domTouchStartEvent.preventDefault(); // prevent panning behaviour
...
class Pressable extends React.PureComponent {
    constructor(props) {
        ...
        this._pointerAreaComponent = createPointerAreaComponent(this);
    ...
    _clearPressState(initialPressState, lastPressState) {
        if (this._isUnmounted) {
            return;
        }

        // call the then-prop if appropriate, before updating state
        if (this.state.initialPressState === initialPressState) {
            this._notifyCompletion(lastPressState);
        }
    ...
    _notifyCompletion(lastPressState) {
        if (this.props.onComplete) {
            this.props.onComplete(lastPressState);
        }

        // @todo deprecate
        if (this.props.then) {
            this.props.then(lastPressState);
        }
    }
    ...
    _startTouchPress(sourceValue, domNode, domTouchStartEvent) {
        ...
        const onMove = (moveEvent) => {
            ...
            // not performing preventDefault() on move event, because some devices disable it anyway,
            // and panning behaviour is already disabled by doing preventDefault() on the start event:
    ...
    render() {
        return this.props.children(this.state.currentPressState, this._pointerAreaComponent);
    }
}
