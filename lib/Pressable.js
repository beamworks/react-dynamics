...
class PressState {
    constructor(value, domNode, domDownEvent, domPosition) {
        this.value = value;
...
function createPressAreaComponent(parent) {
    return class PressArea extends React.PureComponent {
        ...
        _onMouseDown(domNode, domDownEvent) {
            ...
            parent._startMouseState(this.props.value, domNode, domDownEvent);
        }

        _onTouchStart(domNode, domTouchStartEvent) {
            domTouchStartEvent.preventDefault();

            parent._startTouchState(this.props.value, domNode, domTouchStartEvent);
        }
...
class Pressable extends React.PureComponent {
    ...
    _startMouseState(value, domNode, domDownEvent) {
        // mouse event contains the DOM position itself
        const pressState = new PressState(value, domNode, domDownEvent, domDownEvent);
    ...
    _startTouchState(value, domNode, domTouchStartEvent) {
        // touch object has the position info
        const touchObject = domTouchStartEvent.changedTouches[0];
        const pressState = new PressState(value, domNode, domTouchStartEvent, touchObject);
