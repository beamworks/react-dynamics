...
class Task extends React.PureComponent {
    ...
    _activate(activationData) {
        ...
        const promptState = new TaskState(activationData, (result) => {
            ...
            // run the side-effect before changing child state
            if (this.props.then) {
                this.props.then(result);
            }

            // update state (component cannot get unmounted yet from any then-prop side-effect)
            // @todo reducer-style setState?
